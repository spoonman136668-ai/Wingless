...
$Summary = [ordered]@{
    ...
    platform = if ($env:RUNNER_ENVIRONMENT -ceq 'github-hosted') { 'windows-github-hosted-ephemeral' } else { 'windows-self-hosted' }
    ...
    production_priority_guard_mode = if ($env:RUNNER_ENVIRONMENT -ceq 'github-hosted') { 'ephemeral-isolation' } else { 'local-production-priority' }
