...
const UP127CRecurrenceTimingSchema="wingless.up127c-recurrence-timing.v1"

type UP127CPoint struct{
...
type UP127CResult struct{
	Schema string `json:"schema"`
	Experiment string `json:"experiment"`
	SourceUP126CSeal string `json:"source_up126c_seal"`
	...
	Points []UP127CPoint `json:"points"`
}
type up127cEpisode struct{
...
func up127cEpisodeRun(timing string,q2,base,ep int)(out up127cEpisode){
	defer func(){if v:=recover();v!=nil{out.panicHit=true;out.panicMessage=fmt.Sprint(v)}}()
	timingCode:=0;if timing=="recur_after_second"{timingCode=1}
	rng:=newSQ0RNG(sq0Seed(base,6801+q2*263+timingCode*269,ep))
	...
	second:=make([]int,q2);for i:=0;i<q2;i++{second[i]=260+i;truth[second[i]]=rng.intn(32)}
	if timing=="recur_before_second"{
		for _,k:=range targets{up125cPresent(x,k,truth[k],2,&out.validAdmit,&out.maxCurrent,&out.maxHistory)}
		for _,k:=range second{up125cPresent(x,k,truth[k],2,&out.nonAdmit,&out.maxCurrent,&out.maxHistory);out.nonTrials++}
		if x.counter!=0{out.fp+=up125cFill(x,&next,rng,&out.maxCurrent,&out.maxHistory)}
...
func up127cRun(timing string,q2 int,seeds []int) UP127CPoint{
	...
	for _,base:=range seeds{for ep:=0;ep<32;ep++{
		e:=up127cEpisodeRun(timing,q2,base,ep);total++;firstE+=e.firstEvictions;postE+=e.postEvictions
	...
	return UP127CPoint{
		Timing:timing,SecondWaveQualified:q2,PanicRate:rate(panicN,total),PanicMessage:msg,
...
func RunUP127C()(UP127CResult,error){
	res:=UP127CResult{
		Schema:UP127CRecurrenceTimingSchema,Experiment:"UP-127C-recurrence-timing",SourceUP126CSeal:"1b5a64de5f58d08ae37f6d14e92c82f71179da62",
		FirstWaveQualified:16,PostFirstWaveGenerations:2,AdmissionMemoryBytes:128,ExactRecallCap:16,HistoryEntries:32,
	...
	seeds:=[]int{241000000,242000000}
	for _,timing:=range []string{"recur_before_second","recur_after_second"}{
		for _,q:=range []int{0,1,4}{res.Points=append(res.Points,up127cRun(timing,q,seeds))}
	}
	return res,nil
}
