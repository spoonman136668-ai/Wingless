...
const UP127CRecurrenceTimingSchema="wingless.up127c-recurrence-timing-protection.v1"
...
type UP127CResult struct {
	...
	TargetRecurrenceSightings int `json:"target_recurrence_sightings"`
...
func up127cEpisodeRun(timing string,base,ep int)(out up127cEpisode){
	defer func(){ if v:=recover(); v!=nil { out.panicHit=true;out.panicMessage=fmt.Sprint(v) } }()
	rng:=newSQ0RNG(sq0Seed(base,6801,ep))
	...
	if timing=="before_overflow"{recur();overflow()}else{overflow();recur()}
...
func up127cRun(timing string,seeds []int) UP127CPoint{
	...
	for _,base:=range seeds{for ep:=0;ep<32;ep++{
		e:=up127cEpisodeRun(timing,base,ep);total++;firstE+=e.firstEvictions;secondE+=e.secondEvictions
	...
	return UP127CPoint{
		Timing:timing,PanicRate:rate(panicN,total),PanicMessage:msg,
		FirstWaveEvictions:firstE,SecondWaveEvictions:secondE,
		...
		Target16ExactAccuracy:rate(exactHits,total-panicN),
...
func RunUP127C()(UP127CResult,error){
	res:=UP127CResult{
		Schema:UP127CRecurrenceTimingSchema,Experiment:"UP-127C-recurrence-timing-protection",
		SourceUP126CSeal:"1b5a64de5f58d08ae37f6d14e92c82f71179da62",
		FirstWaveQualified:16,SecondWaveQualified:4,TargetRecurrenceSightings:2,
	...
	for _,timing:=range []string{"before_overflow","after_overflow"}{res.Points=append(res.Points,up127cRun(timing,seeds))}
	return res,nil
}
