...
const UP126CSecondWaveSchema="wingless.up126c-second-overflow-wave.v1"

type UP126CPoint struct{
...
type UP126CResult struct{
	Schema string `json:"schema"`
	Experiment string `json:"experiment"`
	SourceUP125CSeal string `json:"source_up125c_seal"`
	...
	Points []UP126CPoint `json:"points"`
}

type up126cEpisode struct{
...
func up126cEpisodeRun(q2,base,ep int)(out up126cEpisode){
	defer func(){if v:=recover();v!=nil{out.panicHit=true;out.panicMessage=fmt.Sprint(v)}}()
	rng:=newSQ0RNG(sq0Seed(base,6701+q2*257,ep))
	...
	beforeSecond:=out.firstEvictions

	second:=make([]int,q2);for i:=0;i<q2;i++{second[i]=260+i}
	...
	if x.counter!=0{out.fp+=up125cFill(x,&next,rng,&out.maxCurrent,&out.maxHistory)}
	out.secondEvictions=x.evictions[0]+x.evictions[1]+x.evictions[2]-beforeSecond
...
func up126cRun(q2 int,seeds []int) UP126CPoint{
	var panicN,total,firstE,secondE int;msg:=""
	...
	for _,base:=range seeds{for ep:=0;ep<32;ep++{
		e:=up126cEpisodeRun(q2,base,ep);total++;firstE+=e.firstEvictions;secondE+=e.secondEvictions
	...
	return UP126CPoint{
		SecondWaveQualified:q2,PanicRate:rate(panicN,total),PanicMessage:msg,FirstWaveEvictions:firstE,SecondWaveEvictions:secondE,
		Age0Evictions:ev[0],Age1Evictions:ev[1],Age2Evictions:ev[2],ValidAdmissionRate:rate(validAdmit,validTrials),
		ValidAccuracy:rate(validHits,validTotal),HotAccuracy:rate(hotHits,hotTotal),Target16Accuracy:rate(targetHits,targetTotal),
		Target16ExactAccuracy:rate(exactHits,total-panicN),NonpersistentAdmissionRate:rate(nonAdmit,nonTrials),
		NonpersistentRetentionRate:rate(nonHits,nonTotal),OneShotFalseAdmissions:fp,MaxCurrentTableEntries:maxCurrent,
		MaxHistoryTableEntries:maxHistory,RecallEntriesUsed:maxRecall,
	}
}
func RunUP126C()(UP126CResult,error){
	res:=UP126CResult{
		Schema:UP126CSecondWaveSchema,Experiment:"UP-126C-second-overflow-wave",SourceUP125CSeal:"9e15a003ab99adf28ebe28777c52d5bf5e86931b",
		FirstWaveQualified:16,AdmissionMemoryBytes:128,ExactRecallCap:16,HistoryEntries:32,
		ReplacementSignal:"maximum_history_age_only",TieBreak:"lowest_table_index",SemanticPriorityUsed:false,QueryPriorityUsed:false,FutureOracleUsed:false,MemoryIncreased:false,
	}
	seeds:=[]int{239000000,240000000};for _,q:=range []int{0,1,4,8}{res.Points=append(res.Points,up126cRun(q,seeds))}
	return res,nil
}
