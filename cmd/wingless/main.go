...
func main() {
	...
	if len(os.Args) > 1 && os.Args[1] == "local-verified-benchmark" {
		if e := runLocalVerified(os.Args[2:]); e != nil {
			fmt.Fprintln(os.Stderr, e)
			os.Exit(1)
		}
		return
	...
	if len(os.Args) != 2 || (os.Args[1] != "demo" && os.Args[1] != "benchmark") {
		fmt.Fprintln(os.Stderr, "usage: wingless host|demo|benchmark|local-benchmark (--config FILE or --endpoint URL --model ID)|local-verified-benchmark (--config FILE or --endpoint URL --model ID)|replay-report --report FILE")
		os.Exit(2)
	}
	reg := &broker.Registry{}
