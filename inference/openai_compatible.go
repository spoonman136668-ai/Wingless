...
func (b *LocalHTTP) Invoke(parent context.Context, r Request) (out Result, err error) {
	...
	payload := chatPayload{Model: b.model, Messages: []map[string]string{{"role": "user", "content": r.Context}}, MaxTokens: r.MaxOutputTokens, Stream: b.streaming, TimingsPerToken: b.streaming}
	if b.streaming {
		payload.StreamOptions = map[string]bool{"include_usage": true}
	}
	if r.OutputConstraint != nil {
		if r.OutputConstraint.Kind != "json_schema" {
			err = fmt.Errorf("output constraint capability unavailable")
			return
		}
		if b.streaming {
			err = fmt.Errorf("output constraint unavailable for streaming")
			return
		}
		format := &responseFormat{Type: "json_schema"}
		format.JSONSchema.Name = "wingless_output"
		format.JSONSchema.Strict = true
		format.JSONSchema.Schema = append(json.RawMessage(nil), r.OutputConstraint.Schema...)
